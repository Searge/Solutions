...
void setup() {
	...
	rings = loadImage(pathDATA + "rings.png");
		...
		.onCreate (
			new HCallback() {
				public void run(Object obj) {
					...
					d.size(5).strokeWeight(0).noStroke().fill(palette[i]).anchorAt(H.CENTER).visibility(true);
					HOrbiter3D orb = new HOrbiter3D(0, 0, 0).target(d).zSpeed(oSpeedZmin).ySpeed(oSpeedYmin).radius(oRadiusMax).zAngle( (int)random(360) ).yAngle( (int)random(360) );
